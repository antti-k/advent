class Intcode:
    ...
    def intcode(self, inputs):
        ...
        def get(location, parameter):
            if parameter == 2:
                return self.relative_base + self.code[location]
            elif parameter == 1:
                return location
            else:
                return self.code[location]
        ...
        while op != 99:
            if op == 1:
                v1 = self.code[get(self.ptr + 1, decoded[1])]
                v2 = self.code[get(self.ptr + 2, decoded[2])]
                pos = get(self.ptr + 3, decoded[3])
                self.code[pos] = v1 + v2
                self.ptr += 4
                ...
                pos = get(self.ptr + 1, decoded[1])
                self.code[pos] = inputs.pop(0)
                self.ptr += 2

            elif op == 4:
                print(self.code[get(self.ptr + 1, decoded[1])])
                self.ptr += 2
                ...
                self.relative_base += self.code[get(self.ptr + 1, decoded[1])]
                self.ptr += 2
